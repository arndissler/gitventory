...
class GitHubAdapterConfig(AdapterConfig):
    ...
    @classmethod
    def token_from_env(cls, v: str) -> str:
        import os
        return v or os.environ.get("GITHUB_TOKEN", "")
...
class GitHubAdapter(AbstractAdapter):
    ...
    def validate_connectivity(self) -> bool:
        ...
        if not cfg.token:
            logger.error(
                "GitHub adapter: no token configured. Set GITHUB_TOKEN or "
                "adapters.github.token in config.yaml."
            )
            return False
        if not cfg.orgs:
    ...
    def _collect_org(self, org: str) -> Iterator[InventoryEntity]:
        ...
        for gh_repo in self._client.list_repos(
            ...
            # Collect GHAS alerts first so we can attach counts to the repo entity
            secret_alerts = []
            code_alerts = []
            dependabot_alerts = []
            ...
            # Yield individual alert entities
            ...
            # Yield OIDC deployment mappings from workflow files
